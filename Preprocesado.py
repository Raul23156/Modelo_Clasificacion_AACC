...
for colegio in os.listdir(ruta_archivos_cortados):
    ...
    for curso in os.listdir(ruta_colegio):
        ...
        for iniciales in os.listdir(ruta_curso):
            ...
            threshold_muscle = 5  # Parámetro ajustable
            ...
            raw_filtered.set_annotations(annot_muscle)  # Añadir anotaciones al objeto
            ...
            if 'Fp1' not in raw.info['bads']:
                ...
                raw_filtered.add_proj(projs)    # Añadir proyecciones al objeto
                raw_filtered.apply_proj()       # Aplicar SSP (atenuación de artefactos oculares)


            ## Reparación de canales malos mediante interpolación esférica
            raw_filtered.interpolate_bads(reset_bads=True, mode='accurate')
            ...
            for band_name, (l_freq, h_freq) in bands.items():
                band_raw = raw_filtered.copy().filter(
                    l_freq=l_freq,
                    h_freq=h_freq,
                    method='fir',
                    fir_design='firwin'
            ...
            for band_name, band_raw in band_signals.items():
                ruta_banda = os.path.join(ruta_archivo_preprocesado, f"banda_{band_name}.fif")  
                band_raw.save(ruta_banda, overwrite=True)